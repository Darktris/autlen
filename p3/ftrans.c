...
char * FtransToString(Ftrans* delta, char* extra, Estado** q, Alfabeto* sigma, int len) {
    ...
    tlen = 0;
    for (i=0; i<len; i++) {
        for (j=0; j<AlfabetoObtieneNumSimbolos(sigma); j++) {
            ...
            res = FtransTransita(delta, q[i], AlfabetoObtieneLetra(sigma, j), &l);
            ...
            l = strlen(extra) + strlen(s1) + strlen(AlfabetoObtieneLetra(sigma, j)) + strlen(s2) + strlen("f(,)={ }\n");
            ...
            s = aux;
            sprintf(s+tlen, "%sf(%s,%s)={ %s}\n", extra, s1, AlfabetoObtieneLetra(sigma, j), s2);
            tlen += l;
            free(s1);
            free(s2);
        }
